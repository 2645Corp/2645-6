...
function uploadFile($file,$filetempname) 
{
    ...
    if($result) //如果上传文件成功，就执行导入excel操作
    {
        ...
        for ($row = 2;$row <= $highestRow;$row++) 
        {
			...
			{
				$result = $DB->query("create table sem".$term['term']."_sj".$id."_exam ( exam char(20) NOT NULL, exam_name char(40), sj_num smallint NOT NULL, class_num smallint NOT NULL, classes varchar(1024) NOT NULL, publisher int NOT NULL, maxgrade smallint NOT NULL, begintime timestamp NOT NULL, endtime timestamp NOT NULL, PRIMARY KEY(exam));");
				$success = $success && $result;
				$result = $DB->query("create table sem".$term['term']."_sj".$id."_grade ( student int, PRIMARY KEY(student));");
				$success = $success && $result;
			}
			if ($result){
				continue;
