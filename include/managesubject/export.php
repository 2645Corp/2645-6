...
//设置表头 
$resultPHPExcel->getActiveSheet()->setCellValue('A1', '科目ID'); 
$resultPHPExcel->getActiveSheet()->setCellValue('B1', '科目名称');  
//设置列宽度
...
$resultPHPExcel->getActiveSheet()->getStyle('A1:B1')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
//设置填充的样式和背景色
...
//读取数据库，生成Excel表格
$i = 2; 
$items= $DB->get_all("select * from subject");
foreach($items as $k=>$v){ 
	$resultPHPExcel->getActiveSheet()->setCellValue('A' . $i, $v['subject']);
	$resultPHPExcel->getActiveSheet()->setCellValue('B' . $i, $v['subject_name']); 
	$i++; 
}
//设置导出文件名 
