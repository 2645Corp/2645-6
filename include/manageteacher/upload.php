        ...
        for ($row = 2;$row <= $highestRow;$row++) 
        {
			...
			if(!empty($subject_a))
			{
				$sql = array(
					...
					'subject' => $strs[2],
					'power' => $strs[3],
					'pwd' => md5(sha1(sha1($strs[0]))));
				$result = $DB->insert('teacher',$sql);
				$success = $success && $result;
			}
			else
			{
				$errorMsg_plus = $errorMsg_plus."Query Error: insert into teacher(teacher,teacher_name,subject,power,pwd) values('".$strs[0]."','".$strs[1]."','".$strs[2]."','".$strs[3]."','".md5(sha1(sha1($strs[0])))."') Invalid value for key 'subject'.";
				$success=false;
				continue;
			}
			if ($result){
