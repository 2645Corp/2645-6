...
//设置表头 
$resultPHPExcel->getActiveSheet()->setCellValue('A1', '班级ID'); 
$resultPHPExcel->getActiveSheet()->setCellValue('B1', '班级名称'); 
$resultPHPExcel->getActiveSheet()->setCellValue('C1', '专业ID'); 
$resultPHPExcel->getActiveSheet()->setCellValue('D1', '专业名称'); 
$resultPHPExcel->getActiveSheet()->setCellValue('E1', '班主任ID');  
$resultPHPExcel->getActiveSheet()->setCellValue('F1', '班主任名称'); 
//设置列宽度
...
//读取数据库，生成Excel表格
$i = 2; 
$items= $DB->get_all("select * from class");
foreach($items as $k=>$v){ 
	$resultPHPExcel->getActiveSheet()->setCellValue('A' . $i, $v['class']);
	$resultPHPExcel->getActiveSheet()->setCellValue('B' . $i, $v['class_name']); 
	$resultPHPExcel->getActiveSheet()->setCellValue('C' . $i, $v['major']); 
	$major=$v['major'];
	$major_a= $DB->get_one("select * from major where major = '$major'");
	$major_name= $major_a['major_name'];
	$resultPHPExcel->getActiveSheet()->setCellValue('D' . $i, $major_name); 
	$resultPHPExcel->getActiveSheet()->setCellValue('E' . $i, $v['headteacher']); 
	...
	$resultPHPExcel->getActiveSheet()->setCellValue('F' . $i, $teacher_name); 
	$i++; 
}
//设置导出文件名 
