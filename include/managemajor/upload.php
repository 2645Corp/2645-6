...
function uploadFile($file,$filetempname) 
{
    ...
    if($result) //如果上传文件成功，就执行导入excel操作
    {
		...
		$success= true;
		$errorMsg_plus= "";
        for ($row = 2;$row <= $highestRow;$row++)  //忽略第一行表头
        {
            $strs=array();
			...
			{
				$result = $DB->query("create table sem".$term['term']."_maj".$id." ( subject smallint NOT NULL, period smallint, score tinyint NOT NULL, maxgrade smallint NOT NULL, PRIMARY KEY (subject) );");
			}
			$success = $success && $result;
			if ($result){
				continue;
