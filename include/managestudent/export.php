...
$resultPHPExcel->getActiveSheet()->setCellValue('E1', '身份证号码'); 
//设置列宽度
...
$resultPHPExcel->getActiveSheet()->getStyle('A1:E1')->getFont()->setBold(true);
//$resultPHPExcel->getActiveSheet()->getStyle('A1:E1')->getFont()->getColor()->setARGB('#FFFF0000');
$resultPHPExcel->getActiveSheet()->getStyle('A1:E1')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
//设置填充的样式和背景色
...
//读取数据库，生成Excel表格
$i = 2; 
$items= $DB->get_all("select * from student");
foreach($items as $k=>$v){ 
	$resultPHPExcel->getActiveSheet()->setCellValue('A' . $i, $v['student']);
	$resultPHPExcel->getActiveSheet()->setCellValue('B' . $i, $v['student_name']); 
	$resultPHPExcel->getActiveSheet()->setCellValue('C' . $i, $v['class']); 
	$resultPHPExcel->getActiveSheet()->setCellValue('D' . $i, $v['gender']); 
	$resultPHPExcel->getActiveSheet()->setCellValue('E' . $i, $v['id']); 
	$resultPHPExcel->getActiveSheet()->setCellValueExplicit('E'.$i,$v['id'],PHPExcel_Cell_DataType::TYPE_STRING);
	$resultPHPExcel->getActiveSheet()->getStyle('C' . $i)->getNumberFormat() ->setFormatCode('000000');
	$i++; 
}
//设置导出文件名 
